...
// check any pending interrupts are now ready to fire
    ...
    current->setExecTime(current->getExecTime() + 1);
...
Interrupt::YieldOnReturn()
{
    //ASSERT(inHandler == TRUE); //if a thread go into the queue, we need to do context switch. However, in this case, inHandler may not be TURE, so I comment this line.
    yieldOnReturn = TRUE;
}
...
void
Interrupt::PrintInt(int number) //implement PrintInt in interrupt.h
{
    kernel->PrintInt(number); //call kernel
}

OpenFileId
Interrupt::Open(char *name) //implement Open in interrupt.h
{
    return kernel->Open(name); //call kernel
}

int
Interrupt::Write(char *buffer, int size, OpenFileId id) //implement Write in interrupt.h
{
    return kernel->Write(buffer, size, id); //call kernel
}

int
Interrupt::Read(char *buffer, int size, OpenFileId id) //implement Read in interrupt.h
{
    return kernel->Read(buffer, size, id); //call kernel
}

int
Interrupt::Close(OpenFileId id) //implement Close in interrupt.h
{
    return kernel->Close(id); //call kernel
}
