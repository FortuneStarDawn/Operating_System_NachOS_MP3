...
void
ConsoleOutput::PrintInt(int number) //implement PrintInt in console.h and this is the final layer
{
    char ch[20] = {};
    sprintf(ch, "%d\n", number); //transform to char type in order to write back
    ASSERT(putBusy == FALSE);
    WriteFile(writeFileNo, ch, sizeof(ch)); //write back
    putBusy = TRUE;
    kernel->interrupt->Schedule(this, ConsoleTime, ConsoleWriteInt);
}
