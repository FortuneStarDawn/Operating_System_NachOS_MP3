...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for(int i=0; i<NumPhysPages; i++) frameTable[i] = FALSE;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
			...
			int temp = ++execfileNum;
			execfile[temp] = argv[++i];
			char *priority = argv[++i];
			ThreadPriorityTable[temp] = atoi(priority);
			cout << execfile[execfileNum] << "\n";
...
void Kernel::ExecAll()
{
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i], ThreadPriorityTable[i]);
	}
...
{
    static int now = 0;
    while(frameTable[now] == TRUE)
    {
        now++;
        if(now == NumPhysPages) now = 0;
    }
    frameTable[now] = TRUE;
    return now;
}

void Kernel::freeFrame(int which)
{
    frameTable[which] = FALSE;
}
