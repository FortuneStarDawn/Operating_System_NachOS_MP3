...
Thread::Yield ()
{
  ...
  if(nextThread->getID() != -1) //ignore postal worker
  {
    cout << "Tick [" << stats->totalTicks << "]: Thread [" << nextThread->getID() << "] is now selected for execution" << endl;
  }
  
  if(this->getID() != -1) //ignore postal worker
  {
    cout << "Tick [" << stats->totalTicks << "]: Thread [" << this->getID() << "] is replaced, and it has executed [" << this->getExecTime() << "] ticks" << endl;
  }
  this->setExecTime(0);
