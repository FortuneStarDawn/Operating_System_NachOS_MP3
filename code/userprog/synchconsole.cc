...
void
SynchConsoleOutput::PrintInt(int number) //implement PrintInt in synchconsole.h and this is called by kernel
{
    lock->Acquire();
    consoleOutput->PrintInt(number); //call PrintInt in console
    waitFor->P();
    lock->Release();
}
