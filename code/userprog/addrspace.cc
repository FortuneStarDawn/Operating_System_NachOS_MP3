...
AddrSpace::~AddrSpace()
{
    ...
    {
        kernel->freeFrame(pageTable[i].physicalPage);
    }
    delete pageTable;
}
...
// Note: this code assumes that virtual address = physical address
    ExceptionType e;
    unsigned int s, va, pa, fa, p;

    s = noffH.code.size;
    va = noffH.code.virtualAddr;
    fa = noffH.code.inFileAddr;
    p = divRoundUp(s, PageSize);

    for(int i=0; i<p; i++)
    {
        e = Translate(va, &pa, 0);
        if(s < PageSize) executable->ReadAt(&(kernel->machine->mainMemory[pa]), s, fa);
        else executable->ReadAt(&(kernel->machine->mainMemory[pa]), PageSize, fa);
        s -= PageSize;
        va += PageSize;
        fa += PageSize;
    }
...
#ifdef RDATA
    s = noffH.readonlyData.size;
    va = noffH.readonlyData.virtualAddr;
    fa = noffH.readonlyData.inFileAddr;
    p = divRoundUp(s, PageSize);
    for(int i=0; i<p; i++)
    {
        e = Translate(va, &pa, 0);
